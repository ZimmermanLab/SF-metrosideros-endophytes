...
otu_table <- otus %>%
  ...
  group_by(otu_id.x, otu_id.y) %>%
  summarize(count = n()) %>%
  spread(otu_id.x, count, fill = 0)
...
tree_coords <- read.csv(paste0("data/metadata/",
...
extraction_with_tree <- read.table(
  "output/processed_sequence_files/groupfile.tsv",
  sep = "\t",
