...
otus <- read.table("output/mothur_pipeline/08_seq_with_OTU_ID.txt",
                   col.names = c("sequence_id", "otu_id"))

groups <- read.table("output/metadata_tables/groupfile.tsv",
                     col.names = c("sequence_id", "tree_id"))
...
culturing <- read.csv("data/metadata/culturing_worksheet_emma.csv")
extractions <- read.csv("data/metadata/extraction_worksheet_emma.csv")

# properly format sequence names in OTU table
otus$sequence_id <- gsub("EUSF00", "EUSF0", otus$sequence_id)
...
# Google API key needed for download
reproject <- FALSE
...
# FIXME Need to only use sequences in analysis
tbas %>%
...
otu_table_combined <- otus %>%
  left_join(groups, by = c("sequence_id" = "sequence_id")) %>%
  mutate(tree_id = gsub("T[0-9]", "", tree_id)) %>%
  group_by(otu_id, tree_id) %>%
  summarize(count = n()) %>%
  spread(otu_id, count, fill = 0)
...
tbas_names_fixed$extraction_id <- gsub(pattern = "EUSF",
                                       replacement = "EUSF0",
                                       tbas_names_fixed$extraction_id)
